...
void benchStringMatcher(const std::string& fileName, uint32 numSamples = 30) {
    ...
    std::vector<uint32> alphabetLengths {2, 64};
    ...
    for (uint32 i = 0; i < alphabetLengths.size(); i++) {
        std::string fName = "/Users/lpraat/develop/string-matching/plot/data/diff/";
...
int main() {
    ...
    benchStringMatcher<BoyerMooreSimple>("boyer_moore_simple.txt");
    return 0;
}